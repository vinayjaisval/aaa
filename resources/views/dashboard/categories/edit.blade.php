                                        ...
                                        @foreach ($fatherSections as $fatherSection)
                                            ...
                                            <option
                                                ...
                                                @foreach ($fatherSection->fatherSections as $subSection)
                                                    ...
                                                    ?>
                                                    <option
                                                        value="{{ $subSection->id  }}" {{ ($subSection->id == $Sections->father_id) ? "selected='selected'":""  }}> &nbsp; {!! (@Helper::currentLanguage()->direction=="rtl")?"&#8617;":"&#8618;" !!} {{ $title }}</option>
                                                @endforeach
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        @foreach(Helper::languagesList() as $ActiveLanguage)
                            @if($ActiveLanguage->box_status)
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        {!! Form::text('title_'.@$ActiveLanguage->code,$Sections->{'title_'.@$ActiveLanguage->code}, array('placeholder' => '','class' => 'form-control','required'=>'','maxlength'=>191, 'dir'=>@$ActiveLanguage->direction)) !!}
                                    </div>
                                </div>
                            @endif
                        @endforeach
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <div id="section_photo" class="col-sm-4 box p-a-xs">
                                                <a target="_blank"
                                                   href="{{ asset('public/uploads/sections/'.$Sections->photo) }}"><img
                                                        src="{{ asset('public/uploads/sections/'.$Sections->photo) }}"
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <div class="input-group">
                                        {!! Form::text('icon',$Sections->icon, array('autocomplete' => 'off','class' => 'form-control icp icp-auto','id'=>'icon', 'data-placement'=>'bottomRight')) !!}
    ...
    <script>
        ...
        $("#seo_url_slug_{{ @$ActiveLanguage->code }}").on('keyup change', function () {
            if ($(this).val() != "") {
                ...
                $("#url_in_engines_{{ @$ActiveLanguage->code }}").text("<?php echo Helper::categoryURL($Sections->id, @$ActiveLanguage->code,$Sections); ?>");
            }
        });
        @endif
        @endforeach
