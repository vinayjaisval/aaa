    ...
    <div class="left navside dark dk" layout="column">
        ...
        <div class="navbar navbar-md no-radius">
            ...
            <a class="navbar-brand" href="{{ route('adminHome') }}">
                @if(Auth::user()->photo !="")
                <img src="{{ asset('public/uploads/users/'.Auth::user()->photo) }}" alt="{{ Auth::user()->name }}"
                    title="{{ Auth::user()->name }}">
                @else
                <img src="{{ asset('public/uploads/contacts/profile.jpg') }}" alt="{{ Auth::user()->name }}"
                    title="{{ Auth::user()->name }}">
                @endif
                ...
                <ul class="nav" ui-nav>
                    ...
                    <li {{ ($GeneralWebmasterSection->id == @$WebmasterSection->id && $is_webmaster != "webmaster") ? 'class=active' : '' }}>
                        ...
                        <ul class="nav-sub">
                            ...
                            $PathCurrentFolder = substr(
                                $urlAfterRoot,
                                (strlen($GeneralWebmasterSection->id) + 1),
                    ...
                    ?>
                    <li {{ ($PathCurrentFolder == $currentFolder) ? 'class=active' : '' }}>
                        <a href="{{ route('contacts_us_form') }}">
                            <span class="nav-icon">
                                <i class="material-icons">&#xe0ba;</i> {{-- contact_mail icon --}}
                            </span>
                    ...
                    @if(Helper::GeneralWebmasterSettings("contacts_status"))
                    @if(@Auth::user()->permissionsGroup->contacts_status)
                    <?php
                    $currentFolder = "contacts"; // Put folder name here
                    $PathCurrentFolder = substr($urlAfterRoot, 0, strlen($currentFolder));
                    ?>
