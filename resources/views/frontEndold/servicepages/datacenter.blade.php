@php
    $language_code = \Helper::currentLanguage()->code;
    $heading  = App\Models\Topic::where(['webmaster_id'=> 30,'status'=>1,'row_no'=>4])->get();
    $cartds  = App\Models\Topic::where(['webmaster_id'=> 30,'status'=>1])->whereIn('row_no', [4])->get();
@endphp
<section>
     <div class="invest-title mt-5">
        <div class="container mt-5">
        @foreach($cartds as $items)
            <div class="row">
                 <div class="col-lg-6">
                    <div class="invest-bussiness">
                        <img src="{{ URL::to('public/uploads/topics/'.$items->photo_file) }}">
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="invest-bussiness-heading">
                       <h2 class="text-uppercase fw-bold">{{ $items->{"title_$language_code"} ?? '' }}</h2>
                       <p  class="cp-text">{!! $items->{"details_$language_code"} ?? '' !!}</p>
                    </div>
                    @endforeach
                     <div class="plus-title mt-4">
                <button class="bg-white border-0  shadow py-2 px-3 rounded text-uppercase fw-bold">Voir la Galerie</button>
            </div>
                </div>
            </div>
        </div>
    </div>
 </section>
 